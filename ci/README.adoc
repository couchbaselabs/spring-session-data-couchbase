== Spring Session for Couchbase CI

Spring Session for Couchbase uses Concourse as it's CI tool of choice. This provides support for:
...
----
% fly -t spring-team sp -p spring-session-data-couchbase -c ci/pipeline-template.yml -l credentials.yml -v branch=master -v release-branch=release
----

With this pipeline in place, you can now activate and expose it:

----
% fly -t spring-team unpause-pipeline -p spring-session-data-couchbase
% fly -t spring-team expose-pipeline -p spring-session-data-couchbase
----
